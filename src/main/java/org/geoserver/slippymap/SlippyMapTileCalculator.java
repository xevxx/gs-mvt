...
public class SlippyMapTileCalculator {
        ...
        switch (srid) {
            case 4326:
                referencedEnvelope = new ReferencedEnvelope(
                        tile2lon(x, zoom), tile2lon(x + 1, zoom), tile2lat(y + 1, zoom), tile2lat(y, zoom), CRS_4326);
                break;
            case 3857:
            case 900913:
                referencedEnvelope = new ReferencedEnvelope(
    ...
    public static int getYTile(final double lat, final double lon, final int zoom) {
        return (int)
                Math.floor((1 - Math.log(Math.tan(Math.toRadians(lat)) + 1 / Math.cos(Math.toRadians(lat))) / Math.PI)
                        / 2
                        * (1 << zoom));
    }
}
